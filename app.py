...
# =============================
# FAQ Matching Enhancements
# =============================

FAQ_SIMILARITY_THRESHOLD = 0.72  # Increased from 0.62 for better precision
FAQ_KEYWORD_BOOST = 0.15  # Additional confidence boost for keyword matches

def enhanced_faq_match(text):
    """
    Enhanced FAQ matching with multiple techniques for better accuracy
    """
    ...
    try:
        ...
        best_faq_question = str(faq_df.iloc[idx]['question'])
        best_faq_answer = str(faq_df.iloc[idx]['answer'])
        
        # Calculate keyword overlap score
        text_tokens = set(clean_text(text).split())
        faq_tokens = set(clean_text(best_faq_question).split())
        
        if text_tokens and faq_tokens:
            intersection = text_tokens.intersection(faq_tokens)
            union = text_tokens.union(faq_tokens)
        ...
        if any(keyword in text.lower() for keyword in best_faq_question.lower().split()):
            combined_score = min(1.0, combined_score + FAQ_KEYWORD_BOOST)
        
        # Additional boost for question-style queries
        if text.strip().endswith('?'):
            combined_score = min(1.0, combined_score + 0.05)
        
        if combined_score >= FAQ_SIMILARITY_THRESHOLD:
            return best_faq_answer, combined_score
        
        return None, combined_score
    except Exception as e:
        st.sidebar.error(f"Error in enhanced FAQ matching: {e}")
        return None, 0.0
    ...
    try:
        if os.path.exists(FAQ_FILE):
            ...
            university_faq = pd.DataFrame({
                ...
                "answer": [
                    "Admission requirements include a completed application, transcripts, test scores, letters of recommendation, and a personal statement.",
                    "Undergraduate tuition is $15,000 per semester. Graduate tuition averages $20,000 per semester.",
                    "We offer a wide range of programs across departments including Computer Science, Business, Engineering, and Psychology.",
                    "Complete the FAFSA using our school code and submit the institutional aid application by March 1st.",
                    "We offer residence halls, suite-style, and apartment-style living with utilities and WiFi included.",
                    "Schedule a campus tour through the Visit Campus section of our website or by calling admissions.",
                    "Priority application deadline for Fall is January 15th. Regular decision is March 1st.",
                    "Reach the admissions office via phone, email, or in-person during office hours.",
                    "The library offers books, databases, e-journals, study rooms, and computer labs.",
                    "Yes, we offer merit-based, need-based, and department-specific scholarships.",
                    "Our student-to-faculty ratio is 15:1 with average class size of 25.",
                    "Facilities include classrooms, labs, recreation center, arts center, dining, and student union.",
                    "Access the student portal using your student ID and password.",
                    "Dining halls, coffee shops, food courts, and convenience stores are available.",
                    "We offer over 100 clubs, sports, cultural events, and leadership programs.",
                    "Computer Science requires 120 credits including core courses in programming and algorithms.",
                    "Yes, we offer online programs in Business, Psychology, Education, and IT.",
                    "Our acceptance rate is around 65%.",
                    "Apply for housing via the student portal and pay a deposit.",
                    "Career services offer resume reviews, interviews, counseling, and job fairs.",
                    "Yes, we offer internships with various industries.",
                    "Undergraduate research opportunities with faculty and grants are available.",
                    "Request transcripts via the student portal or registrar's office.",
                    "Four-year graduation rate is 68%, six-year is 82%.",
                    "Yes, summer courses are available in multiple formats.",
                    "International student services include visa support, advising, and events.",
                    "Change your major by submitting a form to registrar's office.",
                    "Free tutoring is available through the academic success center.",
                    "Connect to Campus-WiFi with your student credentials.",
                    "Health center provides care, immunizations, counseling, and education."
                ]
            })
...
def update_faq_based_on_feedback():
    """
    Analyze feedback to improve FAQ responses
    """
    try:
        feedback_file = os.path.join(DATA_DIR, "faq_feedback.csv")
        ensure_csv(feedback_file, ["timestamp", "question", "provided_answer", "user_feedback", "correct_answer"])
        
        if os.path.exists(feedback_file):
            feedback_df = pd.read_csv(feedback_file)
            negative_feedback = feedback_df[feedback_df["user_feedback"].str.contains("no|incorrect|wrong", case=False, na=False)]
            
            if not negative_feedback.empty:
                problem_questions = negative_feedback["question"].value_counts().head(3)
                for question, count in problem_questions.items():
                    st.sidebar.warning(f"FAQ may need improvement: '{question}' has {count} negative feedback entries")
                    suggested_answers = negative_feedback[negative_feedback["question"] == question]["correct_answer"]
                    if not suggested_answers.empty:
                        most_common_suggestion = suggested_answers.mode()
                        if not most_common_suggestion.empty:
                            st.sidebar.info(f"Suggested improvement: {most_common_suggestion.iloc[0]}")
    except Exception as e:
        st.sidebar.error(f"Error analyzing FAQ feedback: {e}")
...
with tab4:
    st.markdown("<div class='custom-card'>", unsafe_allow_html=True)
    st.write("**FAQ Performance**")
    
    if st.button("Analyze FAQ Performance", key="faq_analysis_btn"):
        update_faq_based_on_feedback()
    
    if os.path.exists(LOG_FILE):
        try:
            df = pd.read_csv(LOG_FILE, on_bad_lines="skip")
            faq_interactions = df[df["predicted_tag"] == "faq"]
            
            if not faq_interactions.empty:
                avg_faq_confidence = faq_interactions["confidence"].mean() * 100
                faq_count = len(faq_interactions)
                positive_feedback = len(faq_interactions[faq_interactions["feedback"].isin(["yes", "Yes", "YES", "1"])])                
                st.metric("Average FAQ Confidence", f"{avg_faq_confidence:.1f}%")
                st.metric("Total FAQ Responses", faq_count)
                st.metric("Positive Feedback Rate", f"{(positive_feedback/faq_count*100):.1f}%" if faq_count > 0 else "N/A")
        except:
            st.info("Could not load FAQ performance data")
    st.markdown("</div>", unsafe_allow_html=True)
